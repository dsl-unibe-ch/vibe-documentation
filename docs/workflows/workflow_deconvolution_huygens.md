...
- Quick **2D visualization**. Once finished, the deconvolution wizard will display the resulting deconvolved image next to the original image to visualize the changes, thus helping the user to decide if the deconvolution operation was successful or if you need to modify your parameters and re-run it again. 

![quick_2d_visualization](../assets/images/huygens_quick_2d_visualizazton.png)


- **Visualization in 3D**: We can explore further the resulting image via 3D visualization. In the main window, select the two images and click on the tab "Visualization". Go to "Visulaization 3D" and select "Twint MIP". This will display the [maximum intensity projection](https://svi.nl/MaxIntensityProjection) (MIP) of the two images in a new window. Sync the vies and readjust contrast or any other parameter to inspect in detail your image. 
